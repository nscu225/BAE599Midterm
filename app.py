...
if page == 'Dataset Inspection & Description':
    ...
    with cols[1]:
        if orig_csv.exists():
            try:
                df_orig_full = pd.read_csv(orig_csv)
                if 'VALUE' in df_orig_full.columns and 'STATE' in df_orig_full.columns:
                    df_box = df_orig_full.copy()
                    df_box['VALUE_num'] = pd.to_numeric(df_box['VALUE'], errors='coerce')
                    top_states = df_box.groupby('STATE')['VALUE_num'].median().dropna().nlargest(12).index
                    df_box_top = df_box[df_box['STATE'].isin(top_states)]
                    fig, ax = plt.subplots(figsize=(4, 3))
                    sns.boxplot(x='STATE', y='VALUE_num', data=df_box_top, ax=ax)
        ...
        try:
            if cleaned.exists():
                ...
                if year_col and val_col:
                    ...
                    st.info('Cleaned CSV missing YEAR or VALUE/YIELD column; cannot draw trend')
            else:
                # fallback: try Step3 image if present
