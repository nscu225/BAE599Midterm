...
if page == 'ML Model Selection':
    ...
    if comp_csv.exists():
        ...
        if df_agg is None or df_agg.empty:
            ...
            # Compute simple model metrics on the small YEAR-level dataset
            try:
                from sklearn.linear_model import LinearRegression
                from sklearn.ensemble import RandomForestRegressor, GradientBoostingRegressor
                from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
            except Exception:
                st.warning('scikit-learn not available in this environment; cannot compute model comparison')
                df_agg = None

        if df_agg is not None and not df_agg.empty:
            X = df_agg[['YEAR']].values
            y = pd.to_numeric(df_agg['VALUE'], errors='coerce').values
            models = {
                'LinearRegression': LinearRegression(),
            }
            # add tree models if available
            try:
                models['RandomForest'] = RandomForestRegressor(n_estimators=50, random_state=0)
                models['GradientBoosting'] = GradientBoostingRegressor(n_estimators=50, random_state=0)
            except Exception:
                pass
            ...
            for name, m in models.items():
                try:
                    ...
                    rows.append({'model': name, 'rmse': None, 'mae': None, 'r2': None})
