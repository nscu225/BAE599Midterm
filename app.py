...
# File uploader to support Streamlit Community Cloud where repo files may not be present
uploaded_file = st.sidebar.file_uploader('Upload original CSV (optional)', type=['csv'])
if uploaded_file is not None:
    # store uploaded bytes in session state for reuse across pages
    try:
        st.session_state['uploaded_orig_df'] = pd.read_csv(uploaded_file)
        st.sidebar.success('Original CSV uploaded and available in-session')
...
def create_cleaned_from_original(df_orig):
    ...
    # Aggregate to one record per year (mean)
    if 'YEAR' in df.columns and 'VALUE' in df.columns:
        df_year = df.groupby('YEAR', as_index=False)['VALUE'].mean()
        # save to Step 2
        ensure_step2_dir()
        outpath = STEP2 / 'cleaned_yield_data.csv'
    ...
    with cols[1]:
        if orig_csv.exists():
            try:
                ...
                state_col = next((c for c in ['STATE','STATE_NAME','State','LOCATION','Location'] if c in df_orig_full.columns), None)
        ...
        try:
            ...
            if not plotted:
                ...
                if orig_df is not None:
                    ...
                    if s3_plot.exists():
                        st.image(str(s3_plot), use_container_width=True, caption='Yield trend with fit')
                        plotted = True
            if not plotted:
...
if page == 'Dataset Preparation':
    ...
    # In-app preprocessing controls (helpful on Streamlit Cloud)
    orig_df_status = get_original_df()
    st.markdown('**In-app preprocessing:** If the repository does not include the original CSV, upload it via the sidebar uploader. You can then run the cleaning step below to create `Step 2/cleaned_yield_data.csv` in-session.')
    if orig_df_status is not None:
        st.success('Original dataset detected (from repo or uploaded file)')
    else:
        st.info('No original dataset detected. Use the sidebar uploader to provide the CSV.')

    if st.button('Run in-app preprocessing (create Step 2/cleaned_yield_data.csv)'):
        if orig_df_status is None:
            st.warning('No original CSV available to process. Upload via the sidebar or add the CSV to the repository.')
        else:
